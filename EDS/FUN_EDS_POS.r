...
# ref data
ref_stat <- readRDS("ref_pos_cpgbeta_meansd.rds")
ref_drift <- readRDS("ref_pos_drift_meansd.rds")
ref_coef <- readRDS("ref_pos_drift_coef.rds")

EDS_POS <- function(input, ref_stat, ref_drift, ref_coef){
		# overlap and missing
		...
		# scale 
		range01 <- function(x){(x-(-23.54058))/(153.9358-(-23.54058))}
